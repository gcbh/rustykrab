...
- Computer-use Phase 3: enigo+xcap backend + CLI wiring (optional feature) (#468)
...
- Add vision (image input) support to the Ollama provider (#460)

## [4.1.3] - 2026-06-01

- Raise snapshot max_depth default to 50 for deep SPAs (#461)

## [4.1.2] - 2026-06-01

- Group caldav under the integration tool category (#459)

## [4.1.1] - 2026-06-01

- feat(recall): persist compaction-displaced history across restarts (#458)
...
- fix(browser): reframe stale-ref recovery payload as observation, not failure (#455)
...
- Remove tool circuit breaker that disabled tools after repeated failures (#451)
...
- fix(tools): bound HttpSession, AdaptiveStore, and SnapshotStore caches (#445)
...
- fix(browser): apply remote_cdp_timeout_ms to Browser::connect calls (#435)
...
- fix(release): explicit versioned libstdc++ link + install diagnostics (#434)
...
- fix(release): add multiarch lib path so zig lld finds libstdc++ (#431)

## [2.10.16] - 2026-05-16

- fix(release): pass multiarch CFLAGS for openssl-sys zigbuild probe (#430)
...
- chore: sync Cargo.lock with v2.10.7 workspace version (#416)
