...
for key in RUSTYKRAB_PROVIDER RUSTYKRAB_PORT RUSTYKRAB_WEB_UI RUST_LOG \
    ...
    RUSTYKRAB_BROWSER_DOWNLOAD_ROOT RUSTYKRAB_BROWSER_BORROW_SYSTEM_PROFILE \
    TELEGRAM_ALLOWED_CHATS TELEGRAM_BOT_TOKEN \
    RUSTYKRAB_PUBLIC_URL RUSTYKRAB_TAILNET_USERS RUSTYKRAB_ALLOWED_ORIGINS \
    RUSTYKRAB_TIMEZONE; do
    if [[ -n "${!key:-}" ]]; then
