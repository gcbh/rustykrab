...
async fn serve_media(State(state): State<AppState>, Path((project_id, filename)): Path<(String, String)>) -> Result<axum::response::Response, StatusCode> {
    ...
    let content_type = match file_path.extension().and_then(|e| e.to_str()).unwrap_or("") {
        ...
        "png" => "image/png", "jpg" | "jpeg" => "image/jpeg", "html" => "text/html", _ => "application/octet-stream",
    };
...
async fn delete_conversation(State(state): State<AppState>, Path(id): Path<Uuid>) -> Result<StatusCode, StatusCode> {
    state.store.conversations().delete(id).map(|_| StatusCode::NO_CONTENT).map_err(|e| match e { rustykrab_core::Error::NotFound(_) => StatusCode::NOT_FOUND, _ => StatusCode::INTERNAL_SERVER_ERROR })
}
...
async fn send_message_stream(State(state): State<AppState>, Path(id): Path<Uuid>, Json(body): Json<SendMessageRequest>) -> Result<Sse<impl tokio_stream::Stream<Item = Result<Event, Infallible>>>, StatusCode> {
    ...
    let agent_handle = tokio::spawn(async move {
        ...
        let on_event = move |event: AgentEvent| {
            ...
            if let Err(e) = event_tx.try_send(SsePayload::Event(event)) { tracing::warn!("SSE event dropped: {e}"); }
        };
        let hb_monitor = heartbeat.clone();
        let tf = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));
        let tf2 = tf.clone();
        let mut monitor = tokio::spawn(async move { const T: u64 = 300_000; loop { tokio::time::sleep(tokio::time::Duration::from_secs(30)).await; let l = hb_monitor.load(std::sync::atomic::Ordering::Relaxed); let n = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis() as u64; if n.saturating_sub(l) > T { tf2.store(true, std::sync::atomic::Ordering::Relaxed); break; } } });
        let agent_future = crate::orchestrate::run_agent_streaming(&agent_state, &mut conv, &user_content, &on_event);
        let result = tokio::select! { r = agent_future => r, _ = &mut monitor => { tracing::warn!("streaming agent timed out"); Err(StatusCode::REQUEST_TIMEOUT) } };
        monitor.abort();
        conv.updated_at = Utc::now();
    ...
    tokio::spawn(async move { if let Err(e) = agent_handle.await { tracing::error!("streaming agent panicked: {e}"); let _ = panic_tx.send(SsePayload::Done(Err(StatusCode::INTERNAL_SERVER_ERROR))).await; } });
    let stream = ReceiverStream::new(rx).map(|p| { let e = match p {
        SsePayload::Event(ae) => match ae {
            AgentEvent::TextDelta(d) => Event::default().event("delta").data(serde_json::json!({"type":"delta","delta":d}).to_string()),
            ...
            AgentEvent::Reflecting => Event::default().event("thinking").data(serde_json::json!({"type":"thinking","delta":"reflecting"}).to_string()),
            AgentEvent::Compressing => Event::default().event("thinking").data(serde_json::json!({"type":"thinking","delta":"compressing"}).to_string()),
        ...
        SsePayload::Done(Err(e)) => { tracing::error!(error=%e,"agent stream error"); Event::default().event("error").data(serde_json::json!({"type":"error","delta":format!("{e}")}).to_string()) }
    }; Ok(e) });
