...
*19 files · 12,156 lines · 159 tests · depends on `core`, `projects`.*
...
in-memory link and card holders, and the database path, and hands out narrow
repository types, one per concern:
...
by every `Store` clone, and is erased when the agent pays, after `CARD_TTL`,
when a newer request in the same conversation supersedes it, or on restart.
`CardDetails` is `Zeroizing` throughout with a redacting `Debug`, and a test
scans the database and WAL bytes for the number, holder and postcode. The
approval and the card source are separate on purpose, so a saved-card wallet
could fill the vault slot later without changing the record. Approving wakes
the filing conversation through `RequestNotifier::payment_authorized`.
