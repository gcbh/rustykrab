...
*18 files · 10,732 lines · 145 tests · depends on `core`, `projects`.*
...
master key (in `Zeroizing`), a credential backend, an optional notifier, and the
database path, and hands out narrow repository types:
...
Credential capture rejects explicitly named payment-card/PAN/CVV fields both when
filing a form and when fulfilling an older pending form. This is a conservative
name/label guard, not comprehensive payment-data detection or a reusable wallet.
No card table, CVV retention mechanism or payment-vault API is introduced.

`inbound.rs` journals channel input before runner admission. An exact UUID/data/
address replay is idempotent; conflicting reuse fails. The journal is separate
from compacted messages. Final conversation saves mark only known retained IDs;
accepted-but-uncheckpointed records stay visible for recovery, never automatic
action replay. A nullable conversation FK cascades bound records on conversation
deletion, so this does not secretly preserve a deleted transcript. Unassigned
records survive process death; reset cancels explicit IDs without deleting data.
Upstream-delivery deduplication, pending-record expiry and a recovery UI are not
