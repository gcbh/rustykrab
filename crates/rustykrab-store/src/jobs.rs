    ...
    pub async fn create_job(
        ...
        with_conn(&self.conn, move |conn| {
            ...
            if !allow_duplicate && !row.one_shot {
                if let Some((existing_id, existing_schedule)) =
                    find_duplicate(conn, &row)?
                {
...
mod tests {
    ...
    fn cron_hour_means_the_operators_hour_not_greenwichs() {
        // The bug this column exists to kill: "0 9 * * *" created by someone
        // in Los Angeles fired at 09:00 UTC, which is 01:00 for them. It has
        // to land on 16:00 UTC — 9am Pacific.
    ...
    fn the_same_expression_holds_its_wall_clock_across_dst() {
        // The reason the *zone* is stored and not the offset it resolved to
        // at creation. One expression, two different UTC instants, because
        // 9am Pacific is 16:00 UTC in July and 17:00 UTC in January. A job
        // created in summer with a frozen -07:00 would start firing at 8am
        // once the clocks went back.
    ...
    fn utc_zone_still_reads_cron_fields_as_utc() {
        // Existing jobs are stamped 'UTC' by the migration, so this is the
        // path that must not move: same expression, same instant as before.
    ...
    fn unknown_stored_zones_fall_back_to_utc_rather_than_failing() {
        ...
        assert_eq!(zone_or_utc(Some("America/Los_Angeles")), LA);
    }
