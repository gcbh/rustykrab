...
| File | Lines | Role |
|---|---|---|
| `main.rs` | 1,963 | Daemon lifecycle, scripted scenarios, report assembly |
...
- 6,361 lines of harness against roughly 84,000 lines of system is a healthy ratio, and
...
- Supporting modules such as surface, assertion, transcript, judge, and
  classification each have one job. `main.rs` still combines process
  supervision, deterministic scenario definitions, and report assembly; it is
  the clear next extraction boundary if the harness grows.
