...
impl AgentRunner {
    ...
    ///
    /// Shares [`Self::run_inner`] with [`Self::run_streaming`], supplying a
    /// sink that discards. Callers that want the events call `run_streaming`;
    /// there is no second loop to fall out of step with this one.
    ...
    ///
    /// There were two of these — `run_inner` and `run_streaming_inner` — 82%
    /// identical, so every behavioural fix had to be made twice, and a fix
    /// applied to only one of them changed behaviour on one transport and not
    /// the other. This is the streaming body, which was the strict superset of
    /// the two; the non-streaming entry point passes a sink that discards, so
    /// all it gives up is events it was never going to deliver.
    async fn run_inner(
        &self,
        conv: &mut Conversation,
        session: &Session,
        ...
        // Track whether any side-effect tool has been executed this run, so
        // we can suppress retries that might duplicate externally-visible
        // actions.
        let mut had_side_effects = false;
        // Track whether the agent has called any tool this run. Gates the
        // "must call task_complete to stop" behavior: a text-only first
        // response (greeting, direct Q&A) still ends the turn, but once the
        // model has started using tools we expect an explicit completion
        // signal.
...
mod tool_choice_guard_tests {
    ...
    /// The claim this file's single loop rests on: `run` and `run_streaming`
    /// do the same thing to a conversation, and differ only in whether the
    /// caller hears about it.
    ///
    /// Before they were merged there were two loop bodies, 82% identical, and
    /// nothing checked that the shared 82% stayed shared. This is that check —
    /// if the two entry points ever diverge again, it fails.
    #[tokio::test]
    async fn both_entry_points_leave_the_same_conversation_behind() {
        ...
        // The difference is the events, and only the events.
        assert!(
