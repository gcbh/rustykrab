...
*14 files · 10,520 lines · 135 tests · depends on `core`, `tools`.*
...
Tool results may also carry images outside their JSON text. The runner records
the concrete model id and whether that provider accepts images before the next
model boundary, making a dropped screenshot distinguishable from bad model
reasoning. Explicit browser CAPTCHA attempts are copied into a privacy-safe
per-run trace. When an outcome sink is enabled, each attempt becomes a separate
implicit outcome record joined to the normal turn by session id, with clearance
kept distinct from downstream task success. Concrete model ids use the existing
`Tool` attribution namespace as `model:<id>` because the current outcome schema
has no model kind; this is queryable but intentionally documented as schema
debt rather than pretending a model is literally a tool.
