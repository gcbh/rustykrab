...
*14 files · 10,812 lines · 138 tests · depends on `core`, `tools`.*
...
| Module | Lines | Role |
|---|---|---|
| `runner.rs` | — | Shared loop, owned interactive completion, admission, compaction, response classification and inline tests |
...
history on failures, caught execution panics and watch-channel cancellation.
Interrupted tool calls receive unknown-outcome results; cancellation aborts local
tool futures but cannot roll back external effects. Admission uses bounded
nonblocking sends, preserves journaled UUIDs and rejects non-user messages.
The loop drains input before iterations and at completion, re-dispatches accepted
follow-ups within one shared iteration cap, and seals admission before exit.
Pending inputs are retained on error/cap paths without claiming they were answered.
Compaction policy and provider trimming are unchanged in this lifecycle split;
their separate context-preservation repair follows in the next PR.
