...
*7 files · 6,136 lines · 111 tests · depends on `core`.*
...
Each network provider also exposes its concrete configured model through
`ModelProvider::model_id`. This is intentionally distinct from `name()` (the
backend family) so observational outcome records can compare `gemma4:26b` with
another model rather than collapsing both into `ollama`. Ollama continues to
resolve vision support from the server-reported capabilities before its model-
family fallback; that gate determines whether browser screenshots actually
cross the provider wire boundary.
