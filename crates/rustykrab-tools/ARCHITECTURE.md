...
*70 files · 29,112 lines · 275 tests · depends on `channels`, `core`, `skills`, `store`.*

<sub>Generated by `scripts/check_architecture_docs.py`. Run it with `--fix` after changing this crate.</sub>
<!-- end:generated-metrics -->

70 files, 29,112 lines, 275 tests — the largest crate in the workspace.
...
| Browser | `browser/` — 10 files | CDP via chromiumoxide: bounded driver requests, profile/session recovery, frame-aware snapshot refs, staged actions, download observation, selectors, stealth, adaptive, fetcher |
...
frame elements use document-local selectors. Detached, over-depth, OOPIF, or
otherwise unreachable contexts are returned as `frames_skipped` rather than
silently described as an empty page. Shadow-root and legacy iframe selector
chains resolve through a shared DOM resolver.
