...
#[derive(Debug, Clone)]
pub struct VideoConfig { pub projects_dir: PathBuf, pub npx_path: String, pub default_quality: String, pub default_format: String, pub env: Vec<(String, String)>, pub mcp_enabled: bool }

impl Default for VideoConfig {
    fn default() -> Self {
        let data_dir = dirs::data_local_dir().unwrap_or_else(|| PathBuf::from(".")).join("rustykrab").join("video");
        Self { projects_dir: data_dir, npx_path: "npx".to_string(), default_quality: "standard".to_string(), default_format: "mp4".to_string(), env: Vec::new(), mcp_enabled: false }
    }
}
...
pub struct VideoChannel { config: VideoConfig, mcp: Arc<Mutex<Option<McpClient>>>, available_tools: Mutex<Vec<McpToolDef>>, env_checked: Mutex<bool> }
...
impl VideoChannel {
    ...
    pub async fn check_environment(&self) -> Result<DoctorResult, String> {
        ...
        *self.env_checked.lock().await = true;
        if result.node_ok && result.ffmpeg_ok { tracing::info!("hyperframes env check passed"); } else { tracing::warn!(node=result.node_ok, ffmpeg=result.ffmpeg_ok, chrome=result.chrome_ok, "deps missing"); }
        Ok(result)
    }
    ...
    pub async fn create_project(&self, name: &str, width: u32, height: u32, duration: f64, fps: u32, template: Option<&str>) -> Result<VideoProject, String> {
        std::fs::create_dir_all(&self.config.projects_dir).map_err(|e| format!("mkdir: {e}"))?;
        ...
        match self.run_npx(&args, Some(&self.config.projects_dir)).await {
            Ok(o) if o.status.success() => { let cd = self.config.projects_dir.join(name); if cd.exists() && cd != project_dir { std::fs::rename(&cd, &project_dir).map_err(|e| format!("rename: {e}"))?; } tracing::info!(project_id=%project_id, template=tmpl, "created via CLI"); }
    ...
    pub async fn add_element(&self, project: &VideoProject, element: &CompositionElement) -> Result<Value, String> {
        ...
        Ok(json!({"status":"added","element_id":element.id,"element_type":element.element_type,"lint":lint.unwrap_or_else(|e| json!({"error":e}))}))
    }
    ...
    pub fn list_projects(&self) -> Result<Vec<VideoProject>, String> {
        ...
        for e in std::fs::read_dir(&self.config.projects_dir).map_err(|e|format!("readdir: {e}"))?.flatten() {
            let p = e.path(); if p.is_dir() { let mp = p.join("project.json"); if mp.exists() { if let Ok(c) = std::fs::read_to_string(&mp) { if let Ok(proj) = serde_json::from_str::<VideoProject>(&c) { ps.push(proj); } } } }
        }
        Ok(ps)
    }
