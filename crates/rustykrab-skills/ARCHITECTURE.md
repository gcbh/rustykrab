...
*6 files · 1,093 lines · 19 tests · depends on `core`.*
...
The baked-in soul's memory-tool guidance is conditional on tool availability;
it no longer unconditionally orders a call to an absent `memory_search`.
Existing operator soul files are not rewritten. The runtime separately states
that the current offered schemas govern invocation, including for custom souls.
